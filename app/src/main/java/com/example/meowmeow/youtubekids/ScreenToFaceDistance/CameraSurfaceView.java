...
import com.example.meowmeow.youtubekids.ScreenToFaceDistance.measurement.Point;
...
import com.example.meowmeow.youtubekids.ScreenToFaceDistance.utils.Util;
...
public class CameraSurfaceView extends SurfaceView implements Callback,Camera.PreviewCallback{
    ...
    _foundFace.getMidPoint(_middlePoint);
            ...
            // portrait mode!
            float heightRatio = canvas.getHeight() / (float) _trackingRectangle.width();
            float widthRatio = canvas.getWidth() / (float) _trackingRectangle.height();
